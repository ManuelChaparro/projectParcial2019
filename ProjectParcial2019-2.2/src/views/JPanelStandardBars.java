...
public class JPanelStandardBars extends JPanel {
	private int zeros = 0;
	...
	public JPanelStandardBars(String[][] matrix) {
...
//		Collections.sort(values, new Utiller());
//		return values;
//	}
...
//		ArrayList<String> localmatrixY = ordinateTable(extractFromMatrix(graficMatrix, 1));
		for (int i = 0; i < getMaxNumberOfMatrix(); i++) {
			double longitudRect = y2AxisY - y1AxisY;
			double increment = (i + 1) * -(longitudRect / getMaxNumberOfMatrix());
...
//		ArrayList<String> localmatrixY = ordinateTable(extractFromMatrix(graficMatrix, 1));
		for (int i = 0; i <= getMaxNumberOfMatrix(); i++) {
			double longitudRect = y2AxisY - y1AxisY;
			double increment = (i) * -(longitudRect / getMaxNumberOfMatrix());
			if (i % (Math.pow(10, zeros - 2)) == 0)
				graphics2d.drawString("" + i, (int) (x2AxisY * 0.5), (int) (y2AxisY + increment));
			CoordinatesY.put("" + i, "" + increment);
		}
	}
